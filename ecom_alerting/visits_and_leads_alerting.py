...
# предположим, что в MS SQL базе лежит словарь посадочных страниц, по которым я хотела бы проверять статистику
# список большой и может меняться, поэтому удобнее его было хранить в виде таблицы в базе
...
query = """
        select url
        from alerting_dict
        where department = 'media_advertising' and product = 'credit' and active = 1
        """
...
# делаем строку, которую можно легко добавить в запрос для clickhouse
...
uri = f"clickhouse://{data['login']}:{data['pass']}@dwh-clickhouse/default"
...
query = f"""
        ...
        from (
            select `ym:s:date` as date,
                   ...
                   if(indexOf(`ym:s:goalsID`, 123) > 0, 1, 0) as flag_start_fill_application,
...
# алёртинг только по URL

def result_df_url(metric):
    
    agg_data = df.groupby(['date', 'startURL']).agg({'visitID':'count', 'flag_start_fill_application': 'sum', 'flag_lead': 'sum'}).reset_index()
    agg_data.rename(columns={'visitID': 'Визиты', 'flag_start_fill_application': 'Приступили к заполнению заявки', 'flag_lead': 'Лиды'}, inplace=True)
...
# алёртинг по URL + breakdown type

def result_df_url_breakdown(metric, agg_type):
    
    agg_data = df.groupby(['date', 'startURL', agg_type]).agg({'visitID':'count', 'flag_start_fill_application': 'sum', 'flag_lead': 'sum'}).reset_index()
    agg_data.rename(columns={'visitID': 'Визиты', 'flag_start_fill_application': 'Приступили к заполнению заявки', 'flag_lead': 'Лиды'}, inplace=True)
    agg_data['date'] = pd.to_datetime(agg_data['date'])
    agg_data['date_upd'] = agg_data['date'].dt.date
...
def send_alert(metric):
    ...
    if metric == 'Визиты':
        ...
        if part:
            part += f"""
            <span style="font-size: 14px; font-family: Helvetica, sans-serif;">
            <p> Данные в таблицах отображаются за вчерашний день. Трекаем количество приступивших к заполнению заявки (или же количество достижений цели dk-landing-form-step-start-to-fill-the-form). </p>
            <p> В разрезе: </p>
            <p> - посадочных страниц (по URL) </p>
            <p> - посадочных страниц + типа устройства (desktop, мобильные устройства + планшеты) </p>
            <p> - посадочных страниц + источника трафика </p>
        ...
        if part:
            part += f"""
            <span style="font-size: 14px; font-family: Helvetica, sans-serif;">
            <p> Данные в таблицах отображаются за вчерашний день. Трекаем количество лидов (или же количество достижений цели Product Page or Landing Credit Application Sent). </p> </p>
            <p> В разрезе: </p>
            <p> - посадочных страниц (по URL) </p>
            <p> - посадочных страниц + типа устройства (desktop, мобильные устройства + планшеты) </p>
            <p> - посадочных страниц + источника трафика </p>
